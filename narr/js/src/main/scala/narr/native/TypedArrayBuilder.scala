 ...
 */
@SerialVersionUID(3L)
class TypedArrayBuilder[T <: TypedArrayPrimitive, AT <: TypedArray[T, AT]](using ct: ClassTag[T], cat: ClassTag[AT])
  extends mutable.ReusableBuilder[T, AT]
    with Serializable {
  protected[this] var capacity: Int = 0
  protected[this] var elems: AT = TArray.ofSize[T](0).asInstanceOf[AT]
...
//  protected[this] def resize(size: Int): Unit
  ...
  private def mkArray(size: Int): AT = {
